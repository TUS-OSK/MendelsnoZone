...
GS = 32 #グリッドサイズ：３２pixel
DOWN,LEFT,RIGHT,UP = 0,1,2,3 #数値割り当て
STOP,MOVE = 0,1  #移動タイプ
PROB_MOVE = 0.005  #移動確率
PROB_ENCOUNT = 0.05  #エンカウント確率
TRANS_COLOR = (190,179,145)  #マップチップの透明色
TITLE, FIELD, TALK, COMMAND, BATTLE_INIT, BATTLE_COMMAND, BATTLE_PROCESS, MOVIE = range(8) #それぞれのゲーム状態への数値割り当て

"""音源と画像"""
sounds = {}  # サウンドset型組み込み型
...
class RPG:
    """RPG要素をこのクラスのインスタンスとしてまとめたRPGクラス"""
    def __init__(self):
        pygame.init()

        # フルスクリーン化 + Hardware Surface未使用
        self.screen = pygame.display.set_mode(SCR_RECT.size, DOUBLEBUF|FULLSCREEN)
        pygame.display.set_caption(u"Mendels no Zone")
        # サウンドをロード
        ...
        # Party()クラスを使ってパーティ
        self.party = Party()
        player1 = Player("young_male1", (3,5), DOWN, True, self.party)
        player2 = Player("young_male2", (3,4), DOWN, False, self.party)
        player3 = Player("thief_female", (3,3), DOWN, False, self.party)
        player4 = Player("butler", (3,2), DOWN, False, self.party)
        ...
        # MessageEngine()クラスを使ってメッセージエンジン
        self.msg_engine = MessageEngine()
        # MessageWindow()クラスを使ってメッセージウィンドウ
        self.msgwnd = MessageWindow(Rect(140,334,360,140), self.msg_engine)
        # CommandWindow()クラスを使ってコマンドウィンドウ
    ...
    def mainloop(self):
        ...
        while True:
            ...
            self.update() #ゲーム状態の更新メソッド

            self.render() #ゲームオブジェクトのレンダリングメソッド
            pygame.display.update() #画面に描画メソッド
            self.check_event() #イベントハンドラメソッド
    ...
    def cmd_handler(self, event):
        ...
        if event.type == KEYDOWN and event.key == K_SPACE:
            ...
            if self.cmdwnd.command == CommandWindow.TALK:  #はなしかける
                sounds["pi"].play()
                self.cmdwnd.hide()
                ...
                if chara != None:
                    ...
                    self.msgwnd.set(u"わたしのまえに　だれもひとがいない。")
                    game_state = TALK

            elif self.cmdwnd.command == CommandWindow.WATCH:  #よくみる
                #ものを調べるコマンド
                sounds["pi"].play()
                self.cmdwnd.hide()
                material = player.watch(self.map)
                if material != None:
                    material.illus()
                    """
                    self.msgwnd.set(u"%sをてにいれた" % treasure.item)
                    game_state = TALK
                    self.map.remove_event(treasure)"""
                else:
                    self.msgwnd.set(u"めのまえには　なんのへんてつもないきょうみのわかないもの　があるだけだ。")
                    game_state = TALK
            ...
            elif self.cmdwnd.command == CommandWindow.SAVE:
                #セーブする
                sounds["pi"].play()
                self.cmdwnd.hide()
    ...
    def battle_cmd_handler(self, event):
        ...
        if event.type == KEYDOWN and event.key == K_SPACE:
            ...
            elif self.battle.cmdwnd.command == BattleCommandWindow.ESCAPE: #マシンをはずす
                self.msgwnd.set(u"わたしは　マシンをはずした。")
...
class CommandWindow(Window):
    ...
    COMMAND = [u"ココまでおきたことを　きおくにとどめる", u"", u"", u"",
               u"", u"", u"", u"マシンをはずす"]
    def __init__(self, rect, msg_engine):
        Window.__init__(self, rect)
...
class Title:
    ...
    def __init__(self, msg_engine):
        self.msg_engine = msg_engine
        self.title_img = load_image("data", "mendelsnozone.png", -1)
    ...
    def draw(self, screen):
        ...
        #self.msg_engine.draw_string(screen, (260,320), u"")
        #クレジットの描画
        self.msg_engine.draw_string(screen, (130,400), u"PRESS ESCAPEKEY TO EXIT")
        #メニューカーソルの描画
...
class Battle:
    ...
    def start(self):
        ...
        for bsw in self.status_wnd:
            bsw.hide()
        self.msgwnd.set(u"わたしは　はしをのぼっているおとこ　にであった。")
        self.play_bgm()
    def update(self):
        pass
    def draw(self, screen):
...
class BattleCommandWindow(Window):
    ...
    COMMAND = [u"はなしをきく", u"ゆめみる", u"おもいだす", u"マシンをはずす"]
    def __init__(self, rect, msg_engine):
        Window.__init__(self, rect)
